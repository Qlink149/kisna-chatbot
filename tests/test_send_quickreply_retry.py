...
class TestSendQuickreplyWithRetry(unittest.TestCase):
    @patch("kisna_chatbot.whatsapp_functions.quick_reply.send_quick_reply.time.sleep")
    @patch(
        "kisna_chatbot.whatsapp_functions.quick_reply.send_quick_reply.httpx.post"
    )
    def test_succeeds_first_try_no_sleep(self, post_mock, sleep_mock):
    ...
    def test_retries_on_500_then_succeeds(self, post_mock, sleep_mock):
        ...
        sleep_mock.assert_called_once_with(1)  # 2**0

    @patch("kisna_chatbot.whatsapp_functions.quick_reply.send_quick_reply.time.sleep")
    @patch(
        "kisna_chatbot.whatsapp_functions.quick_reply.send_quick_reply.httpx.post"
    )
    def test_retries_on_network_error(self, post_mock, sleep_mock):
    ...
    def test_exhausts_retries_and_raises_last_error(self, post_mock, sleep_mock):
        ...
        self.assertEqual(sleep_mock.call_count, 2)  # not slept after the last attempt
