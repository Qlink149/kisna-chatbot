...
def send_video_call_request_flow(phone_number: str):
    ...
    if not flow_id:
        logger.warning(
            "KISNA_VIDEOCALL_FLOW_ID not set — skipping video call flow send",
            extra={"phone_number": phone_number},
        )
        return None

    logger.info(
        "Sending video call request flow",
        extra={"phone_number": phone_number},
    )
    ...
    data = {
        ...
        "interactive": {
            ...
            "body": {"text": "Please share your mobile number and preferred time."},
            "footer": {"text": "Kisna"},
            "action": {
                "name": "flow",
                "parameters": {
    ...
    try:
        ...
        body = response.json()
        logger.info(
            "Video call request flow API response",
        ...
        logger.error(
            "Error sending video call request flow",
